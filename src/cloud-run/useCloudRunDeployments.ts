...
export const useCloudRunDeployments = (projectId: string): UseCloudRunDeploymentsResult => {
  const { services, isLoading: isLoadingServices } = useCloudRunServices(projectId);
  const { jobs, isLoading: isLoadingJobs } = useCloudRunJobs(projectId);
  const { workerPools, isLoading: isLoadingWorkerPools } = useCloudRunWorkerPools(projectId);
